...
def cast_columns_to_types(df, type_mapping):
    ...
    for col, target_dtype in type_mapping.items():
        ...
        try:
            ...
            conversion_log[col] = {'from': str(original_dtype), 'to': str(target_dtype), 'status': 'success'}
            print(f"✓ {col}: {original_dtype} → {target_dtype}")
        except Exception as e:
            conversion_log[col] = {'from': str(original_dtype), 'to': str(target_dtype), 'status': 'failed', 'error': str(e)}
...
def convert_currency_to_float(df, currency_columns):
    ...
    for col in currency_columns:
        if col not in df.columns:
            ...
            continue
        try:
            # Handle empty strings and NaNs safely before replacing
            df_typed[col] = df_typed[col].fillna('')
            df_typed[col] = df_typed[col].astype(str).str.replace(r'[$,]', '', regex=True).str.strip()
            # Convert to empty string back to NaN, then to float
            df_typed[col] = pd.to_numeric(df_typed[col].replace('', np.nan), errors='coerce')
    ...
    for col in boolean_columns:
        ...
        try:
            ...
            if df[col].dtype == 'object':
                mapping = {'yes': True, 'no': False, 'y': True, 'n': False, 'true': True, 'false': False, '1': True, '0': False, 1: True, 0: False, True: True, False: False}
                df_typed[col] = df_typed[col].str.lower().map(mapping) # Convert string to lower case for mapping
            else:
...
if __name__ == "__main__":
    ...
    df_typed = convert_string_dates_to_datetime(df_typed, ['transaction_date', 'signup_date'], date_format='%Y-%m-%d')
